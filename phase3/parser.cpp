...
static void globalDeclarator(int typespec)
{
	...
	match(']');
	    ...
	    declareVariable(name, Type(typespec, indirection)); //SCALAR
    }
}
...
static void globalOrFunction()
{
	...
	if (lookahead == '{') {
		defineFunction(name, Type(typespec, indirection, param));
	    match('{');
	    declarations();
	    statements();
	    closeScope();
		...
		closeScope();
		declareFunction(name, Type(typespec, indirection, param));
		remainingDeclarators(typespec);
	}
    } else {
	declareVariable(name, Type(typespec, indirection)); //SCALAR
